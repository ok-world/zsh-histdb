...
* News
- 18/01/21 :: Added table and fixed SQL query to save username (from SSH key at .ssh/authorized_keys). You have to declare 2 environment variables SSH_HASH and SSH_NAME to work properly.
Add to the ~.zshrc~ file few command rows:
#+BEGIN_SRC zsh
typeset -g SSH_SES=`echo $SSH_CONNECTION | awk '{print $2}'`
typeset -g SSH_HASH=`tail /var/log/sshdusers.log | grep ${SSH_SES} |  awk '{ print $16 }'`
typeset -g SSH_NAME=`ssh-keygen -l -f $HOME/.ssh/authorized_keys | grep $SSH_HASH | awk '{ name=$3" "$4; print name }'`
